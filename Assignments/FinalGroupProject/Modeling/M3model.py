...
def empiricalVariteArrial():
    ...
    if (k==0):
        return 0
    return W[k]+((r-F[k-1])/(F[k]-F[k-1]))*(W[k]-W[k-1])

def empiricalVariteSerivce():
    W=SW
    F=SF
    r=random.random()
    k=0
...
AW=loadData('ArrialW.txt')
AF=loadData('ArrialF.txt')
SW=loadData('ServiceW.txt')
SF=loadData('ServiceF.txt')
